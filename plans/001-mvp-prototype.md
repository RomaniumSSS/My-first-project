...
2.  **Минимальный Бот:**
    - [x] `src/main.py`: Запуск поллинга.
    - [x] `src/bot/handlers/start.py`: Простой хендлер `/start` -> "Бот работает".
3.  **Деплой/Запуск:**
...
1.  **FSM Onboarding:**
    - [x] Сбор данных: Имя -> Главная цель (Текст).
2.  **FSM Goal Setting:**
    - [x] Команда `/new_goal`.
    - [x] Прием описания и опционально фото (Мудборд).
    - [x] Конвертация фото в base64 и сохранение в БД (или временно в память для анализа, если база будет пухнуть, но для прототипа пишем в БД).
    - [x] **AI Анализ**: Отправка данных в сервис -> Получение мотивирующего ответа.
...
1.  **Команда `/checkin`:**
    - [x] Бот предлагает выбрать активную цель (Inline кнопки).
    - [x] Пользователь пишет отчет (текст/фото).
2.  **Реакция AI:**
    - [x] Анализ отчета по сравнению с целью.
    - [x] Генерация фидбека (похвала + совет).
    - [x] Сохранение в историю.

## Этап 5: Альфа-тестирование
Проверка на реальных людях.

1.  **Подготовка:**
    - [x] Инструкция для тестеров (что нажимать, что ожидать) -> `README.md`.
    - [x] Выдача доступа (white-list или открытый доступ) -> `src/main.py` middleware + `config.py`.
2.  **Сбор обратной связи:**
    - [x] Логирование ошибок в файл/консоль -> `bot.log` + `stdout`.
    - [x] Ручной сбор фидбека от пользователей -> Инструкции в `README.md`.
