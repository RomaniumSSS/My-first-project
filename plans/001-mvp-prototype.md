...
1.  **OpenAI Client Wrapper (`src/services/ai.py`):**
    - [ ] Настройка клиента `AsyncOpenAI`.
    - [ ] **Retry & Timeout**: Использовать `tenacity` для повторных попыток при ошибках сети/5xx (3 попытки, экспоненциальная задержка). Таймаут запроса 30-60 сек.
    - [ ] **Fallback**: Если API недоступен после ретраев -> возвращать заглушку "Мозг коуча сейчас перезагружается, попробуй позже".
    - [ ] **Логирование**: Записывать промпт (сокращенно), статус ответа и latency.
2.  **Vision Helper:**
    - [ ] Функция скачивания фото из Telegram -> конвертация в Base64.
    - [ ] Подготовка payload для GPT-5.1.
