...
from src.services.vision import download_telegram_photo, encode_image_to_base64, prepare_vision_payload
...
async def process_title(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Хорошо. Теперь опиши подробнее: почему это важно? Как ты поймешь, что цель достигнута?"
    )
...
async def finalize_goal(message: types.Message, state: FSMContext, photo_base64: str | None):
    """
    ...
    processing_msg = await message.answer("Анализирую твою цель и готовлю план действий...")

    # Prepare AI Prompt
    system_prompt = (
        "Ты — опытный и эмпатичный коуч. Твоя задача — вдохновить пользователя и дать 3 первых шага к цели."
    )
    user_text = f"Моя цель: {title}\nОписание: {description}\n\nДай мне мотивирующий пинок и 3 простых шага для начала."
    ...
    # Get AI Response
    # AICODE-NOTE: We call AI before saving to DB to fail fast if AI is down? 
    # No, plan says "Convert photo -> DB -> AI". Better to save first to not lose user input.
    ...
    try:
        ...
        ai_response = "Цель сохранена! К сожалению, мой AI-модуль сейчас недоступен, но я верю в тебя! Начни с малого."
