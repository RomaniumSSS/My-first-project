...
from src.bot.states import OnboardingStates, GoalSettingStates, CheckInStates, CrisisStates, ReflectStates
...
def get_main_menu_keyboard(has_goals: bool = False):
    ...
    builder.button(text="🎯 Новая цель", callback_data="menu_new_goal")
    if has_goals:
        builder.button(text="✅ Чек-ин", callback_data="menu_checkin")
    builder.button(text="🧘 Рефлексия", callback_data="menu_reflect")
    builder.button(text="🆘 Кризис", callback_data="menu_crisis")
    builder.adjust(2)
...
@router.callback_query(F.data == "menu_new_goal")
async def handle_menu_new_goal(callback: types.CallbackQuery, state: FSMContext):
    """Переход к созданию новой цели."""
    user = await User.get_or_none(telegram_id=callback.from_user.id)
    if not user:
        await callback.answer("Сначала нужно познакомиться! Нажми /start", show_alert=True)
        return
    ...
    await state.clear()
    await callback.message.answer("Давай поставим новую цель! Как она звучит? (Заголовок)")
    await state.set_state(GoalSettingStates.waiting_for_title)


@router.callback_query(F.data == "menu_checkin")
async def handle_menu_checkin(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Выбери цель для отчета:", reply_markup=builder.as_markup())
    await state.set_state(CheckInStates.waiting_for_goal_selection)


@router.callback_query(F.data == "menu_reflect")
...
@router.callback_query(F.data == "menu_crisis")
